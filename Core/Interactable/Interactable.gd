...
export var interact_parent : NodePath
...
func _on_InteractionComponent_body_entered(body):
	...
	if (body.has_method("interaction_can_interact")):
		...
		canInteract = body.interaction_can_interact(get_node(interact_parent))
	...
	emit_signal("on_interact_changed", interaction_target)


func _on_InteractionComponent_body_exited(body):
	if (body == interaction_target):
		interaction_target = null
		emit_signal("on_interact_changed", null)
